...
class TestFormValidation:
    ...
    def test_register_form_valid_data(self, app_context):
        ...
        with patch('flask.current_app.config.get', side_effect=lambda key, default=None: True if key == 'DISABLE_CAPTCHA' else default), \
             app_context.test_request_context():  # Disable CAPTCHA for test
    ...
    def test_register_form_password_mismatch(self, app_context):
        ...
        with patch('flask.current_app.config.get', side_effect=lambda key, default=None: True if key == 'DISABLE_CAPTCHA' else default), \
             ...
             app_context.test_request_context():  # Disable CAPTCHA
...
class TestFormSecurity:
    ...
    def test_input_sanitization(self, app_context):
        ...
        with patch('db.schema.User.query') as mock_query, \
             patch('flask.current_app.config.get', side_effect=lambda key, default=None: True if key == 'DISABLE_CAPTCHA' else default), \
...
class TestFormRendering:
    ...
    def test_register_form_renders_correctly(self, client):
        ...
        assert b'confirm' in response.data or b'password2' in response.data or b'repeat' in response.data.lower()
...
class TestFieldValidators:
    ...
    def test_username_length_validation(self, app_context):
        ...
        with app_context.test_request_context(), \
            ...
            assert 'characters' in error_message.lower() or 'length' in error_message.lower()
