...
- Point of contact is the undersigned at (719) 555-0123."""
    ...
    def wait_for_progress_modal(self, driver, should_be_visible=True, timeout=10):
        ...
        if should_be_visible:
            return WebDriverWait(driver, timeout).until(
                EC.visibility_of_element_located((By.ID, "progress-modal"))
            )
        else:
            return WebDriverWait(driver, timeout).until(
                EC.invisibility_of_element_located((By.ID, "progress-modal"))
            )
    ...
    def test_pdf_generation_home_page(self, mock_celery_task, driver, app_url, sample_memo_content):
        ...
        assert progress_text in ["Processing...", "0%"], f"Unexpected initial progress: {progress_text}"
        
        # Verify Celery task was called
        mock_celery_task.assert_called_once()
        
        # Check that form data was submitted correctly
        call_args = mock_celery_task.call_args[1] if mock_celery_task.call_args[1] else mock_celery_task.call_args[0]
        assert sample_memo_content.replace('\n', '\r\n') in str(call_args) or sample_memo_content in str(call_args)
    ...
    def test_pdf_generation_form_view(self, mock_celery_task, driver, app_url):
        ...
        submit_button = self.wait_for_clickable(driver, By.CSS_SELECTOR, "input[type='submit'], button[type='submit']")
    ...
    @patch("app.create_memo.delay")
    def test_progress_bar_no_nan_error(self, mock_celery_task, driver, app_url, sample_memo_content):
        ...
        while time.time() - start_time < 5:  # Monitor for 5 seconds
            ...
            assert "NaN%" not in progress_text, f"Found NaN% in progress text: {progress_text}"
...
class TestPDFGenerationIntegration:
    ...
    def server_running(self):
        ...
        pytest.skip("Development server not running. Start with: docker-compose -f docker-compose-dev.yaml up")
